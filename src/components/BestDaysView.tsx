  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="p-4 bg-white border border-zinc-200 rounded-2xl">
        <h3 className="text-sm font-bold text-zinc-800 mb-2.5 flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-1.5 p-1 bg-zinc-100 rounded-xl border border-zinc-200">
          <button
            ...
            className={`flex-1 text-center py-1.5 px-2 text-[11px] font-bold rounded-lg transition-all ${
              bestDayFilter === 'ALL_ACTIVE' ? 'bg-orange-500 text-black shadow-sm font-black' : 'text-zinc-500 hover:text-zinc-700'
            }`}
          >
            Alle tilgjengelige
          </button>
          <button
            ...
            className={`text-center py-1.5 px-2.5 text-[11px] font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${
              bestDayFilter === 'WITHOUT_KIDS' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-xs' : 'text-zinc-500 hover:text-emerald-600'
            }`}
          >
            🍻 Uten barn
          </button>
          <button
            ...
            className={`text-center py-1.5 px-2.5 text-[11px] font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${
              bestDayFilter === 'WITH_KIDS' ? 'bg-blue-100 text-blue-800 border border-blue-200 shadow-xs' : 'text-zinc-500 hover:text-blue-600'
            }`}
          >
            👶 Med barn
          </button>
          <button
            ...
            className={`text-center py-1.5 px-2.5 text-[11px] font-bold rounded-lg transition-all flex items-center justify-center gap-1 ${
              bestDayFilter === 'VACATION' ? 'bg-amber-100 text-amber-800 border border-amber-200 shadow-xs' : 'text-zinc-500 hover:text-amber-600'
            }`}
          >
            🏖️ På ferie
          </button>
      ...
      {sortedDays.length === 0 ? (
        <div className="text-center py-12 px-4 bg-white border border-zinc-200 rounded-2xl">
          <Calendar className="w-10 h-10 text-zinc-400 mx-auto mb-3" />
          <h4 className="text-sm font-semibold text-zinc-600">Ingen dager med tilgjengelige venner</h4>
        ...
        <div className="space-y-2.5">
          {sortedDays.map((day, idx) => {
            ...
            return (
              <div
                key={day.dateStr}
                className="w-full text-left p-3.5 bg-white border border-zinc-200 rounded-2xl flex items-center justify-between transition-all group max-w-full"
              >
                <div className="flex items-center gap-3.5 min-w-0 pr-2">
                  <div className="w-9 h-9 rounded-xl bg-zinc-100 border border-zinc-200 flex flex-col items-center justify-center text-zinc-500 group-hover:border-zinc-300 transition-colors shrink-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="text-sm font-bold text-zinc-800 group-hover:text-black transition-colors">
